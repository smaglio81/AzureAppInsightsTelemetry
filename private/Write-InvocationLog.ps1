...
#>
    ...
    {
        if ($param.Key -in ($global:AzureAppInsightsTelemetry.AlwaysExcludeParametersForLogging + $ExcludeParameter))
        {
            continue
        }

        if ($param.Key -in ($global:AzureAppInsightsTelemetry.AlwaysRedactParametersForLogging + $RedactParameter))
        {
