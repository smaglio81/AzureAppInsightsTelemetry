...
#>
    ...
    Begin
    {
		...
		if([string]::IsNullOrWhiteSpace($Path)) {
			$Path = $global:AzureAppInsightsTelemetry.LogPath
		}
    }
    ...
    End
    {
        ...
        if ($global:AzureAppInsightsTelemetry.LogTimeAsUtc)
        {
        ...
        if ($global:AzureAppInsightsTelemetryLogProcessId)
        {
        ...
        try
        {
            if (-not $global:AzureAppInsightsTelemetry.DisableLogging)
            {
                if ([String]::IsNullOrWhiteSpace($Path))
                {
